#!/usr/bin/env python3
"""Create a retirement post-mortem from the bundled template."""
...
def main() -> int:
    ...
    archive_dir = root / 'research' / 'superquants' / 'archive'
    ...
    output_path = archive_dir / f'{args.date}-{slug}-post-mortem.md'
    write_text_file(output_path, render_template('post-mortem-template.md', replacements), force=args.force)

    ensure_seed(archive_dir / 'INDEX.md', 'archive-index-template.md')

    print(f'Created post-mortem: {output_path}')
    print('Remember to add the strategy to the archive index with its cause of death.')
    return 0
