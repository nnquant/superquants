#!/usr/bin/env python3
"""Create a minimal failing slice document from the bundled template."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--slug', help='ASCII slug for the strategy or issue.')
    parser.add_argument('--title', help='Human-readable title for the issue.')
...
def main() -> int:
    ...
    diagnoses_dir = root / 'research' / 'superquants' / 'diagnoses'
    ...
    output_path = diagnoses_dir / f'{args.date}-{slug}-minimal-failing-slice.md'
    write_text_file(output_path, render_template('minimal-failing-slice-template.md', replacements), force=args.force)

    print(f'Created minimal failing slice: {output_path}')
    return 0
