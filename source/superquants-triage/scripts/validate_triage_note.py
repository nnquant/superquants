#!/usr/bin/env python3
"""Validate that a Triage note contains the required sections."""
...
def main() -> int:
    ...
    if missing:
        print(f'Triage note is missing required sections in {path}:')
        for heading in missing:
            print(f'- {heading}')
        return 1

    print(f'Triage note looks complete: {path}')
    return 0
